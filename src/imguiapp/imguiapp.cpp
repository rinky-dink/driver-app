...
void ShowFilesInFolder(const std::string& folderPath) {
	//int fileCount = std::count_if(std::filesystem::directory_iterator(folderPath), std::filesystem::directory_iterator(),
	//	[](const std::filesystem::directory_entry& entry) {
	//		return entry.is_regular_file();
	//	});

	//ImGui::Text("%s %d", folderPath.c_str(), fileCount);
...
int main(int argc, char** argv)
{
	...
	if (semaphore == NULL) {
		...
		if (argc >= 2)
		{
			...
			if (!server)
			{
				...
				//message sda{ "sdadadadadada", "sdadadadadada sda" };
				//
				///*for (int i = 1; i < argc; i++) {
				//	sda.x += argv[i];
				//	sda.x += " ";
				//		if (strcmp("game", argv[i]) == 0 && i + 1 < argc)
				//		{
				//			sda.y = argv[++i];
				//		}
				//	}
				...
				//std::cout << sda.x << std::endl;


				//std::cout << sda.y << std::endl;

				//std::cin.get();*/

				//std::cout << "sizeof(sda) " << sizeof(sda) << std::endl;

				//message sdad{ "sdadadadadada", "sdadadadadada" };

				//std::cout << "sizeof(sdad) " << sizeof(sdad) << std::endl;

				//std::cout << sda.x.length() << sda.y.length() << std::endl;
				//
				//std::cin.get();
	...
	std::thread r{ [&] {
		while (!done) {
			...
			//// Проверяем, что размер полученного буфера соответствует размеру структуры
			//if (buf.size() == sizeof(message)) {
			//	// Получаем указатель на данные в буфере
			//	const message* msg = reinterpret_cast<const message*>(buf.data());

			//	// Проверяем содержимое структуры
			//	if (msg->x == "game") {
			//		// Ваш код обработки события "game"
			//	}
			...
			// Создание stringstream для разделения строки
			std::stringstream ss(dat);

			// Разделение строки и добавление подстрок в вектор
	...
	{
		bool assertstatus = driver.Install();
		assert(assertstatus == false);
	}
	...
	/*ImFont* myFont;
	LoadFontFromResource(myFont);*/

	ImFontConfig config;
	config.MergeMode = false; // Вы можете настроить это на ваше усмотрение
	...
	while (!done)
	{
		...
		//	// Добавление текста в консоль (пример)
		//	
