...
)
{
	...
	{
		DbgPrint("RtlQueryRegistryValues failed, using default values, 0x%x\n", ntStatus);
	}
	return ntStatus;
}
...
BOOLEAN WriteRegistryValue()
{
	...
	{
		DbgPrint("Failed to write registry value: 0x%X\n", status);
		return FALSE;
	}

	DbgPrint("Registry value written successfully\n");
	return TRUE;
}
//
...
NTSTATUS DriverDeviceControl(
	...
	UNREFERENCED_PARAMETER(DeviceObject);
	DbgPrint("Entered Device Control MJ Function\n");
	...
	{
		DbgPrint("BLUESTREET_SEND_DATA Control Activated\n");
...
)
{
	...
	//WCHAR BufferDllPathX86[2][128];
	//WCHAR BufferDllPathX64[2][128];

	//WCHAR path[2][128];
	//for (size_t i = 0; i < 2; i++)
	//{
	//	g_Settings.Path[i].Length = 0;
	//	g_Settings.Path[i].MaximumLength = sizeof(path[i]);
	//	g_Settings.Path[i].Buffer = path[i];
	//}
	//WCHAR dlls[128];
	//g_Settings.Dlls.Length = 0;
	//g_Settings.Dlls.MaximumLength = sizeof(dlls);
	//g_Settings.Dlls.Buffer = dlls;

	////Status = UpdateSettings();
	////if (!NT_SUCCESS(Status)) {
	////	return Status;
	////}
	///*Status = InjInitialize(g_DriverObject, g_RegistryPath, &g_Settings);
	//if (!NT_SUCCESS(Status)) {
	//	return Status;
	//}*/
	...
	//#define _M_IX86 = 1;
	//#if defined (_M_IX86)
	//g_Settings.Method = InjMethodThunk;
